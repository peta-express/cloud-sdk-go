...
generate: snips ../petaexpress-api-specs/package.json
	./snips \
		-f=../petaexpress-api-specs/2013-08-30/swagger/api_v2.0.json \
		-t=./template \
		-o=./service
	go fmt ./service/...
	@echo "ok"
...
snips:
	curl -L https://github.com/raksmart/snips/releases/download/v0.2.16/snips-v0.2.16-${shell go env GOOS}_amd64.tar.gz | tar zx

../petaexpress-api-specs/package.json:
	-go get -d github.com/raksmart/petaexpress-api-specs
	file %@
...
unit-coverage:
	...
	for pkg in ${PKGS_TO_CHECK}; do \
		output="coverage$${pkg#github.com/peta-express/cloud-sdk-go}"; \
		mkdir -p $${output}; \
...
ADD . /go/src/github.com/peta-express/cloud-sdk-go
WORKDIR /go/src/github.com/peta-express/cloud-sdk-go
